...
#include <iostream>
#include <string>
#include <stdexcept> //Заголовок <stdexcept> определяет ряд стандартных исключений, которые могут быть сгенерированы библиотечными функциями C++ или системой времени выполнения.
#include <ctime> //Включает время заголовка <стандартной библиотеки C.h> и добавляет связанные имена в std пространство имен.
#include <locale> 
//Определяет шаблоны классов и функции, которые программы C++ могут 
//использовать для инкапсулирования и управления различными культурными соглашениями относительно представления и форматирования
 //классификации символов и сортировки строк.
...
private:
    ErrorCode code;//переменная может принимать одно значение из enum
};
